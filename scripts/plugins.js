...
const get_config = (key) => {
    if (hexo.config.theme_config && hexo.config.theme_config.hasOwnProperty(key)) {
        return hexo.config.theme_config[key];
    } else {
        return hexo.theme.config[key];
    }
}
...
const source = (path, cache, ext) => {
    if (cache) {
        ...
        return get_config('cdn') ? `//unpkg.com/${name}@latest${minFile}` : `${minFile}?v=${version}`;
    } else {
        return path + ext;
    }
}
...
hexo.extend.tag.register('image', ([src, alt = '', title = '']) => {
    return get_config('lightbox') ? renderImage(src, alt, title) : `<img src="${src}" alt="${alt}" title="${title}">`;
});

hexo.extend.filter.register('before_post_render', data => {
    if (get_config('lightbox')) {
